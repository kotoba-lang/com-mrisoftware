(ns mrisoftware.main
  "Kotodama WASM entrypoint for the mrisoftware clean-room actor (L4) — Clojure port.
  ...
  facts (`mrisoftware.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mrisoftware")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "mrisoftw_acc"
    :fields [:holderId :type :balance :currency :status]         :required [:type :balance]
    :coerce {:balance :float}                                    :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "mrisoftw_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"mrisoftware.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mrisoftware-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mrisoftware-compat/ready)
